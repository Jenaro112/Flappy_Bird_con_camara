...
if MEDIAPIPE_NUEVO:
    ...
    # ── MODELO DE MANOS ─────────────────────────────────────
    HAND_MODEL_PATH = _ruta("models", "hand_landmarker.task")
...
EMOJIS = {
    "topo_gigio": render_emoji("\U0001F42D"),  # Ratón (Topo Gigio)
...
def overlay_foto_lado(frame, nombre_base, max_w=550, max_h=550):
    ...
    en el margen derecho del video, con tamaño máximo max_w x max_h.
    Si la imagen tiene canal alpha, hace una mezcla transparente.
    """
    for ext in [".jpg", ".png", ".jpeg"]:
        for prefijo in ["assets", ""]:
            ruta = _ruta(prefijo, nombre_base + ext) if prefijo else _ruta(nombre_base + ext)
...
# ── CÁMARA ──────────────────────────────────────────────────
# Prioridad: índices 1, 2 primero para evitar Continuity Camera del iPhone
cap = None
...
for idx in [1, 2, 0, 3]:
    ...
    if temp_cap.isOpened():
        ...
        if success and frame_test is not None and frame_test.any():
            cap = temp_cap
            print(f"Cámara seleccionada: índice {idx}")
            break
        temp_cap.release()
...
GESTOS = {
    "topo_gigio": {"emoji": "\U0001F42D", "label": "TOPO GIGIO!",  "foto": "riquelme", "color": (0, 255, 255)},
